  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8 space-y-6">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
        <p className="text-sm font-semibold text-slate-700 mb-2">New entry</p>
        <textarea
          value={text}
      ...
      <div className="space-y-3">
        {entries.length === 0 && (
          <p className="text-sm text-slate-400 text-center">No journal entries yet.</p>
        )}
        {entries.map((e) => (
          <div key={e.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
            ...
            <p className="text-sm text-slate-700 whitespace-pre-wrap">{e.text}</p>
            {e.trigger && (
