...
function StatCard({ label, value, sub }) {
  return (
    <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-ink">ClearDay Admin</h1>
        <button onClick={onLogout} className="text-sm text-faint hover:text-red-500">Log out</button>
      </div>
      ...
      <div className="flex gap-1 mb-6">
        {[["overview", "Overview"], ["review", `Review${pendingCount ? ` (${pendingCount})` : ""}`], ["users", "Users"]].map(([id, label]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${tab === id ? "bg-slate-800 text-white" : "text-muted hover:bg-subtlebg"}`}
          >
            {label}
          </button>
        ))}
      ...
      {tab === "overview" && (
        <div className="space-y-6">
          ...
          <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-5">
            <p className="text-sm font-semibold text-ink mb-4">Signups, last 7 days</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
              <p className="text-xs text-faint uppercase tracking-wide mb-2">By addiction</p>
              {Object.entries(stats.byAddiction).map(([a, c]) => (
                <div key={a} className="flex justify-between text-sm py-0.5">
                  <span className="text-muted">{ADDICTION_LABELS[a] || a}</span>
                  <span className="font-semibold text-ink">{c}</span>
                </div>
              ))}
            </div>
            <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
              <p className="text-xs text-faint uppercase tracking-wide mb-2">Engagement</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
              <p className="text-xs text-faint uppercase tracking-wide mb-2">Support providers</p>
              <div className="flex justify-between text-sm py-0.5"><span className="text-muted">Verified</span><span className="font-semibold text-ink">{stats.providerCounts.verified}</span></div>
              <div className="flex justify-between text-sm py-0.5"><span className="text-muted">Pending</span><span className="font-semibold text-amber-600">{stats.providerCounts.pending}</span></div>
            </div>
            <div className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
              <p className="text-xs text-faint uppercase tracking-wide mb-2">Institutions</p>
              <div className="flex justify-between text-sm py-0.5"><span className="text-muted">Verified</span><span className="font-semibold text-ink">{stats.institutionCounts.verified}</span></div>
              <div className="flex justify-between text-sm py-0.5"><span className="text-muted">Pending</span><span className="font-semibold text-amber-600">{stats.institutionCounts.pending}</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === "review" && (
        <div className="space-y-6">
          <div>
            <p className="text-sm font-semibold text-ink mb-3">Pending provider applications</p>
            {pendingProviders.length === 0 && <p className="text-sm text-faint">Nothing pending.</p>}
            <div className="space-y-3">
              {pendingProviders.map((p) => (
                <div key={p.id} className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
                  <p className="font-semibold text-ink">{p.displayName} <span className="text-xs font-normal text-faint">· {p.phone}</span></p>
                  <p className="text-xs text-brand-600 font-medium mt-0.5">{p.specialty}</p>
                  <p className="text-sm text-muted mt-2">{p.bio}</p>
                  {p.credentials && <p className="text-xs text-faint mt-1">Credentials: {p.credentials}</p>}
                  <div className="flex gap-2 mt-3">
                    ...
                    <button onClick={() => rejectProvider(p.id)} className="flex-1 bg-subtlebg hover:bg-red-50 hover:text-red-600 text-muted text-sm font-semibold py-2 rounded-lg transition">Reject</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <p className="text-sm font-semibold text-ink mb-3">Pending institution applications</p>
            {pendingInstitutions.length === 0 && <p className="text-sm text-faint">Nothing pending.</p>}
            <div className="space-y-3">
              {pendingInstitutions.map((inst) => (
                <div key={inst.id} className="bg-surface rounded-2xl shadow-sm border border-subtle p-4">
                  <p className="font-semibold text-ink">{inst.name}</p>
                  <p className="text-xs text-faint mt-0.5">{inst.type} {inst.contactPhone && `· ${inst.contactPhone}`}</p>
      ...
      {tab === "users" && (
        <div className="bg-surface rounded-2xl shadow-sm border border-subtle overflow-hidden">
          {!users ? (
            <p className="text-sm text-faint p-4">Loading...</p>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-subtlebg text-muted text-xs uppercase">
                <tr>
